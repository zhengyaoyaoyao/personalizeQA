...
public abstract class AbstractFileStrategy implements FileStrategy {
    ...
    public InfoSourceFile uploadInfoSourceFile(MultipartFile multipartFile, String taskCode) {
        try {
            ...
            InfoSourceFile infoSourceFile = InfoSourceFile.builder().dataType(FileDataTypeUtil.getDataType(multipartFile.getContentType()))
                    ...
                    .ext(FileNameUtil.extName(multipartFile.getOriginalFilename())).build();
            uploadInfoSourceFileImpl(infoSourceFile,multipartFile);
    ...
    public abstract void uploadInfoSourceFileImpl(InfoSourceFile infoSourceFile,MultipartFile multipartFile) throws Exception;

    @Override
    public InfoSourceAttach uploadInfoSourceAttach(MultipartFile multipartFile, String taskCode) {
        try {
            if (!multipartFile.getOriginalFilename().contains(FILE_SPLIT)){
                log.debug("缺少后缀名");
            }
            InfoSourceAttach infoSourceAttach = InfoSourceAttach.builder().dataType(FileDataTypeUtil.getDataType(multipartFile.getContentType()))
                    .submittedFileName(multipartFile.getOriginalFilename())
                    .isDelete(false)
                    .taskCode(taskCode)
                    .size(multipartFile.getSize())
                    .contextType(multipartFile.getContentType())
                    .ext(FileNameUtil.extName(multipartFile.getOriginalFilename()))
                    .build();
            uploadInfoSourceAttachImpl(infoSourceAttach,multipartFile);
            return infoSourceAttach;
        }catch (Exception e){
            log.error("上传文件失败{}",e);
            throw BizException.wrap(ExceptionCode.BASE_VALID_PARAM.build("文件上传失败"));
        }
    }
    public abstract void uploadInfoSourceAttachImpl(InfoSourceAttach infoSourceAttach,MultipartFile multipartFile) throws Exception;
